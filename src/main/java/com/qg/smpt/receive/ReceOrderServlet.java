...
public class ReceOrderServlet extends HttpServlet {
    ...
    public void doGet(Integer userId, Order order,int printerId) throws ServletException, IOException {
        // 获取商家id
//        Integer userId = null;
//        try {
...
//        if (printers == null || printers.size() <= 0) { return ; }
        ...
        Printer printer = selectPrinter(user.getPrinters(),printerId);
        if (printer == null) {
...
//        while(true) {
            ...
            for (Printer p : printers) {
                if (p.isConnected() && p.isCanAccept() && p.getId()==printerId) {
                    return p;
                }
            }
